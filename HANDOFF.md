...
Owner: Michael Dalton (President). Both WPCNA form paths use the WPCNA-controlled inbox `wpcna.info@gmail.com` as the default recipient and also deliver to `michael@mdalton.com` plus `michael.kushman@gmail.com`. Contact Us uses FormSubmit CCs; Community Posting merges all three recipients in the Worker. Site email links display `info@wpcna.org`, while their underlying `mailto:` BCCs `michael@mdalton.com` (configured via `site.emailMailto` in `src/_data/site.js`).
